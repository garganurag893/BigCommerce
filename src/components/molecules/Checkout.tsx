...
const Checkout = () => {
  const { selectedProducts, addressDetails } = useCheckoutStore(
    (state) => ({
  ...
  const { firstName, lastName, streetAddress, zip, state, city } = addressDetails || {}
  ...
  return (
    <>
      ...
      <ul className='flex flex-row flex-wrap justify-start my-4'>{renderSelectedProducts()}</ul>
